...
namespace Assets.Scripts.OpenStreetMap
{
    public class ItemEnumerator
    {
        ...
        public static nodeType getNodeType(OsmElement node)
        {
            if (node.Tags.IsNullOrEmpty())
                return nodeType.None;

            if (node.Tags.ContainsKey("natural") && node.Tags.ContainsKey("tree"))
            {
                return nodeType.Tree;
            }
            ...
            if (node.Tags.ContainsKey("amenity"))
            {
                ...
                if (node.Tags["amenity"] == "post_box")
                    return nodeType.PostBox;
                if (node.Tags["amenity"] == "telephone")
                    return nodeType.PhoneBox;
            }
            ...
            if (node.Tags.ContainsKey("highway"))
            {
                if (node.Tags["highway"] == "traffic_signals")
                    return nodeType.TrafficSign;
                if (node.Tags["crossing"] == "traffic_signals")
                    return nodeType.TrafficSign;
                if (node.Tags["highway"] == "street_lamp")
                    return nodeType.StreetLamp;
            }
