...
namespace Assets.Scripts.SceneObjects
{
    ...
    public class Building
    {
        ...
        public Building(Way _way, BuildingConfigurations config, Material mat, int materialID, float texWidth)
        {
            ...
            building.transform.SetParent(CityConstructor.Instance.city.transform);
        ...
        public void RenderBuilding()
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError("Error construct building "+ (type == buildingType.standard? id: relation.outerWall.id));
        ...
        private void createFacade(int facadeID, Node node1, Node node2,GameObject building)
        {
            ...
            if (node1.meterPosition.IsNaN() || node2.meterPosition.IsNaN()|| float.IsNaN( equalizedBuildingHeight) || float.IsInfinity(equalizedBuildingHeight))
            {
                return;
            }
            ...
            if (mesh.vertices.Length < 3 || mesh.triangles.Length == 0)
            {
                Debug.Log("Highway Mesh is empty " + facade.name);
            }
            else
            {
        ...
        private void createRoof(Way way,GameObject roof)
        {
            ...
            if (float.IsNaN(equalizedBuildingHeight) || float.IsInfinity(equalizedBuildingHeight))
            {
                return;
            }
            for (int i = 0; i < vertices.Length; i++)
            {
                if (vertices2D[i].IsNaN() || vertices2D[i].IsNaN() )
                {
                    return;
                }
        ...
        private float getBuildingHeight(List<Tag> tagList)
        {
            ...
            for(int i =0 ; i < tagList.Count ; i++)
            {
                if (tagList[i].k == "building:level")
                    _buildingHeight = float.Parse(tagList[i].v, OSMparser.culture) * 3.0f;
                else if (tagList[i].k == "building:height")
                    _buildingHeight = float.Parse(tagList[i].v, OSMparser.culture);
