...
namespace Assets.Scripts.SceneObjects
{
    class DefaultObject3DHandler
    {
        ...
        public static Object3D drawTrafficSign(Vector3 pos)
        {
            ...
            obj.object3D.transform.SetParent(CityConstructor.Instance.city.transform);
        ...
        private static Object3D drawTree(Node nd)
        {
            ...
            obj.type = ObjectType.Tree;
            obj.resourcePath = "Prefabs/Environment/Prefabs/BroadLeafMobile";
            obj.object3D = (GameObject)MonoBehaviour.Instantiate(Resources.Load(obj.resourcePath));
            obj.object3D.AddComponent<Object3dMouseHandler>();
            obj.object3D.transform.position = nd.meterPosition;
            obj.object3D.transform.SetParent(CityConstructor.Instance.city.transform);
            // set random rotation Y
            obj.object3D.transform.Rotate(new Vector3(0, UnityEngine.Random.Range(0, 360), 0));
            // set random scale with min 0.8 and max 1.5
            const float minInclusive = 0.8f;
            const float maxInclusive = 1.2f;
            obj.object3D.transform.localScale = new Vector3(UnityEngine.Random.Range(minInclusive, maxInclusive), UnityEngine.Random.Range(minInclusive, maxInclusive), UnityEngine.Random.Range(minInclusive, maxInclusive));
