...
namespace Assets.Scripts.SceneObjects
{
    ...
    public class Highway
    {
        ...
        public Highway(Way w, List<HighwayConfigurations> config, myTerrain _terrain)
        {
            ...
            highwayGameObject.transform.SetParent(CityConstructor.Instance.city.transform);
        ...
        public void renderHighway()
        {
            ...
            for (int k = 0, m = 0; k < vertices.Length; k += 2, m++)
            {
                ...
                if (vertices[k].IsNotValid() || vertices[k+1].IsNotValid())
                {
                    return;
                }
            }
            ...
            {
                Debug.Log("Highway Mesh is empty "+ highwayGameObject.name);
                return;
            }
...
{
    public static bool IsNotValid(this Vector3 v)
    {
